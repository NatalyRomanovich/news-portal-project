...
public class UserDAOImpl implements UserDAO {
	...
	public boolean registration(NewUserInfo user) throws DaoException {
		...
		try (Connection connection = ConnectionPool.getInstance().takeConnection();
			...
			// RegisrtationCheckInDB regisrtationCheckInDB = new
			// RegisrtationCheckInDB(connection, user);

			/*
			 * if (regisrtationCheckInDB.isUsedLogin() ||
			 * regisrtationCheckInDB.isUsedEmail()) { return false; }
			 */
	...
	/*
	 * @Override public List<String> getAlreadyUsedParameters(NewUserInfo user)
	 * throws DaoException {
	 * 
	 * try (Connection connection = ConnectionPool.getInstance().takeConnection();
	 * PreparedStatement ps = connection.prepareStatement(ADDING_USER)) {
	 * 
	 * RegisrtationCheckInDB alreadyUsedParameters = new
	 * RegisrtationCheckInDB(connection, user); List<String>
	 * alreadyUsedParametersList = new ArrayList<String>();
	 * 
	 * if (alreadyUsedParameters.isUsedLogin()) {
	 * alreadyUsedParametersList.add(UsersParameter.LOGIN_PARAM); }
	 * 
	 * if (alreadyUsedParameters.isUsedEmail()) {
	 * alreadyUsedParametersList.add(UsersParameter.EMAIL_PARAM);
	 * 
	 * } System.out.println("dao dataForRegistration " + alreadyUsedParametersList);
	 * return alreadyUsedParametersList;
	 * 
	 * } catch (SQLException e) { throw new
	 * DaoException("Something is wrong with the database", e);
	 * 
	 * } catch (ConnectionPoolException e) { throw new
	 * DaoException("Connection is not established", e); } }
	 */
	...
	/*private class RegisrtationCheckInDB {
		...
		public RegisrtationCheckInDB(Connection connection, NewUserInfo user) throws SQLException, DaoException {
			this.connection = connection;
			this.user = user;
			this.usedLogin = isLoginUsed(user);
			this.usedEmail = isEmaillUsed(user);
		}
