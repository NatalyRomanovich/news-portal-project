...
public class CompletelyDeleteNewsFromDB implements Command {

	private final NewsService newsService = ServiceProvider.getInstance().getNewsService();
	private final static Logger LOG = LogManager.getLogger(CompletelyDeleteNewsFromDB.class);
		...
		try {
			...
			if (requestGetSession == null) {
				...
				if (!role.equals(UsersRole.ADMIN.getTitle())) {
					...
					String idNews = request.getParameter(NewsParameter.JSP_ID_NEWS);

					if (newsService.deleteNewsFromDB(idNews)) {
